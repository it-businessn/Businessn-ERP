import { FormControl, FormLabel, HStack, Input, VStack } from "@chakra-ui/react";
import TextTitle from "components/ui/text/TextTitle";

export const OtherMoniesInfoReview = ({ formData }) => {
	return (
		<>
			<TextTitle size="xl" title={"Other Monies"} />
			{formData?.employmentInfo?.otherMonies?.map((om, i) => (
				<VStack key={i} align="stretch" spacing={2} borderWidth="1px" p={3} borderRadius="md">
					<HStack key={i} spacing={2} align="end">
						{/* Code */}
						<FormControl>
							<FormLabel>Code</FormLabel>
							<Input value={om.code} />
						</FormControl>

						{/* Start Date */}
						<FormControl>
							<FormLabel>Start Date</FormLabel>
							<Input type="date" value={om.startDate} />
						</FormControl>

						{/* End Date */}
						<FormControl>
							<FormLabel>End Date</FormLabel>
							<Input type="date" value={om.endDate} />
						</FormControl>

						{/* Amount */}
						<FormControl>
							<FormLabel>Amount</FormLabel>
							<Input type="number" step="0.01" value={om.amount} />
						</FormControl>
					</HStack>
				</VStack>
			))}
		</>
	);
};
