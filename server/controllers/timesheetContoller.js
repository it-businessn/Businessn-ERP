    ...
    try {
        ...
        if (filteredData?.filteredEmployees?.length) {
            timesheets = timesheets.filter((item) =>
                filteredData?.filteredEmployees?.includes(
        ...
        if (filteredData?.filteredDept?.length) {
            timesheets = timesheets.filter((item) =>
                filteredData?.filteredDept?.includes(
                    item.employeeId.department[0]
                )
            );
        }
    ...
    const employeeEarnings = timesheets?.reduce((acc, item) => {
        return (
            ...
            (item?.regHoursWorked * empPayInfoResult?.regPay +
                ...
                item?.dblOvertimeHoursWorked *
                    empPayInfoResult?.dblOverTimePay +
...
const calcTotalWorkedHours = (clockIn, clockOut) => {
    if (clockOut) {
        ...
        const finalTime = totalTime.toFixed(2).includes(".99")
...
const deleteTimesheet = async (req, res) => {
    ...
    try {
        ...
        if (timesheet) {
            res.status(200).json(
                `Timesheet with id ${id} deleted successfully.`
            );
        } else {
